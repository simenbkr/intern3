...
class ArbeidListe extends Liste {
	...
	public static function alleSammen ($Start = false , $Lengde = false) { // Forelda pga teit navn og feil parametre
		if (is_int($Lengde)) {
			if (!is_numeric($Start)) {
			$Start = 0;
			}
	...
	public static function Antall () { // Denne trengs vel ikke når alleSammen() er borte og $Sideinndeling er på plass.
